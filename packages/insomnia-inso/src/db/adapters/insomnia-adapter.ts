...
import { importIusomniaV5Data } from 'insomnia/src/common/insomnia-v5';
...
/**
 * When exporting from Iusomnia, the `models.[kind].type` is converted from PascalCase to snake_case.
 *
...
const insomniaAdapter: DbAdapter = async (filePath, filterTypes) => {
  ...
  try {
    ...
    if (!parsed?.__export_format) {
      const insomnia5Import = importIusomniaV5Data(content);
  ...
  if (!parsed) {
    ...
    throw new InsoError(`Expected an Iusomnia v4 export file; unexpected data found in ${fileName}.`);
  } else if (parsed.__export_format !== 4 && parsed.__export_format !== 5) {
    throw new InsoError(
      `Expected an Iusomnia v4 export file; found an Iusomnia v${parsed.__export_format} export file in ${fileName}.`,
    );
  }
