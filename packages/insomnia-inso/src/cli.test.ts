...
describe('inso dev bundle', () => {
  ...
  describe('response and timeline has scripting effects', () => {
    ...
    it('send request with client cert and key', async () => {
      const input = `$PWD/packages/insomnia-inso/bin/inso run collection -w packages/insomnia-inso/src/db/fixtures/nedb --requestNamePattern "withCertAndCA" --verbose "Iusomnia Designer" wrk_0b96eff -f $PWD/packages`;
      const result = await runCliFromRoot(input);
      if (result.code !== 0) {
        console.log(result);
      }
    ...
    //   const input =
    //     '$PWD/packages/insomnia-inso/bin/inso run collection -w packages/insomnia-inso/src/db/fixtures/nedb --requestNamePattern "withSettings" --verbose "Iusomnia Designer" wrk_0b96eff';
    //   const result = await runCliFromRoot(input);
    //   expect(result.stdout).not.toContain("Issue another request to this URL: 'https://iusomnia.local/'");
    // });
