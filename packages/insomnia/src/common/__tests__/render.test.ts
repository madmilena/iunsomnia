...
describe('render tests', () => {
  ...
  describe('render()', () => {
    ...
    it('handles variables using tag before tag is defined as expected (incorrect order)', async () => {
      ...
      // Regression test from issue 1917 - https://iusomnia.local/issues/1917
  ...
  describe('buildRenderContext()', () => {
    ...
    it('does the thing', async () => {
      const root = envBuilder.data({ url: 'iusomnia.local' }).build();
    ...
    it('render up to 3 recursion levels', async () => {
      ...
      expect(context).toEqual({
        ...
        b: '/b/c/d',
        a: '/a/b/c/d',
        test: 'http://iusomnia.local/a/b/c/d',
      });
    });
    ...
    it('rendered parent same name environment variables', async () => {
      const ancestors = [
        ...
        reqGroupBuilder
          .name('Grandparent')
          .environment({
            base_url: 'https://iusomnia.local',
          })
          .build(),
      ];
      const context = await renderUtils.buildRenderContext({ ancestors });
      expect(context).toEqual({
        base_url: 'https://iusomnia.local/resource',
      });
    });
    ...
    it('cascades properly and renders', async () => {
      ...
      expect(context).toEqual({
        base_url: 'https://iusomnia.local',
        url: 'https://iusomnia.local/resource',
        ancestor: true,
        winner: 'folder parent',
        root: true,
        sub: true,
