...
export const disclaimer =
  'Iusomnia provides features that may automatically execute code. Only import files from trusted sources.';
...
const IusomniaIcon = (props: React.SVGProps<SVGSVGElement>) => {
  return (
...
export const SupportedFormats = () => {
  ...
  return (
    <div>
      <div className="pb-(--padding-sm)">{t('modals.supportedFormats')}</div>
  ...
  if (!scanResults.length) {
    return (
      <div className="flex items-center gap-(--padding-sm) border border-solid border-(--hl-md) bg-(--hl-xxs) p-(--padding-sm)">
        <i className="fa-regular fa-file fa-lg" />
        {t('modals.noValidResourcesFound')}
      </div>
    );
  }
                ...
                <td className="bg-(--hl-xxs)">
                  ...
                  >
                    {hasErrors ? (
                      <Fragment>
                        <i className="fa-regular fa-file fa-lg" />
                        {t('modals.parseFileFailedPrefix')} {scanResult.oriFileName}{' '}
                        {t('modals.parseFileFailedSuffix')}
                      </Fragment>
                    ) : (
                      <Fragment>
                        {getImporterSign(scanResult)} {t('modals.resourcesToBeImportedFrom')}{' '}
                  ...
                  {scanResult.workspaces &&
                    ...
                    getWorkspaceCountsByScope(scanResult.workspaces, scanResult).map(({ scope, count }) => (
                      <tr key={scope} className="table--no-outline-row">
                        <td>
                          {count} {scopeToLabelMap[scope as ProjectScopeKeys] ?? t('common.workspace')}
                          {count > 1 ? 's' : ''}
                        </td>
                      </tr>
                    ))}
                  {scanResult.requests && scanResult.requests?.length > 0 && (
                    <tr key={scanResult.requests[0]._id} className="table--no-outline-row">
                      <td>
                        ...
                        {t(scanResult.requests.length === 1 ? 'common.request' : 'common.requests')}
                      </td>
                    </tr>
                  )}
                    ...
                    <tr key={scanResult.apiSpecs[0]._id} className="table--no-outline-row">
                      <td>
                        <div className="flex items-center gap-(--padding-md)">
                          ...
                          {t(scanResult.apiSpecs.length === 1 ? 'modals.openapiSpec' : 'modals.openapiSpecs')}
                        </div>
                      </td>
                    </tr>
                  )}
                  {scanResult.environments && scanResult.environments.length > 0 && (
                    <tr className="table--no-outline-row">
                      <td>
                        ...
                        {t(
                          scanResult.environments.length === 1 ? 'common.environment' : 'common.environments',
                        )}
                        {' '}
                        {t('modals.with')}{' '}
                        {scanResult.cookieJars?.length}{' '}
                        {t(scanResult.cookieJars?.length === 1 ? 'modals.cookieJar' : 'modals.cookieJars')}
                      </td>
                    </tr>
                  )}
                  {scanResult.unitTestSuites && scanResult.unitTestSuites?.length > 0 && (
                    <tr className="table--no-outline-row">
                      <td>
                        ...
                        {t(scanResult.unitTestSuites.length === 1 ? 'modals.testSuite' : 'modals.testSuites')}
                        {' '}
                        {t('modals.with')}{' '}
                        {scanResult.unitTests?.length}{' '}
                        {t(scanResult.unitTests?.length === 1 ? 'modals.test' : 'modals.tests')}
                      </td>
                    </tr>
                  )}
                  {scanResult.mockRoutes && scanResult.mockRoutes?.length > 0 && (
                    <tr className="table--no-outline-row">
                      <td>
                        ...
                        {t(scanResult.mockRoutes?.length === 1 ? 'modals.mockRoute' : 'modals.mockRoutes')}
                      </td>
                    </tr>
                  )}
                </Fragment>
...
function getImporterSign(scanResult: ScanResult) {
  ...
  if (scanResult.type?.id) {
    ...
    if (id.includes('insomnia')) {
      icon = <IusomniaIcon width={24} height={24} />;
      name = 'Iusomnia';
