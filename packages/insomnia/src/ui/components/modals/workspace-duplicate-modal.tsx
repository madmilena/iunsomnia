  ...
  return (
    <OverlayContainer onClick={e => e.stopPropagation()}>
      <Modal onHide={onHide} ref={modalRef}>
        <ModalHeader>{t('modals.duplicateFile')}</ModalHeader>
        <ModalBody className="wide">
          <p className="mb-6">{t('modals.duplicateFileToProject')}</p>
          ...
          >
            ...
            <div className="form-control form-control--outlined">
              <label>
                {t('modals.newWorkspaceName', { workspaceLabel: getWorkspaceLabel(workspace).singular.toLowerCase() })}
        ...
        <ModalFooter>
          <div>
            ...
            <button disabled={isBtnDisabled} form="workspace-duplicate-form" className="btn">
              {fetcher.state !== 'idle' && <Icon icon="spinner" className="animate-spin" />} {t('common.duplicate')}
            </button>
          </div>
        </ModalFooter>
      </Modal>
