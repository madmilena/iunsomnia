...
import { getAppWebsiteBaseURL, getIusomniaPublicKey, getIusomniaSecretKey } from '../common/constants';
...
encodeBase64(sessionKeyPair.publicKey).then(res => {
  try {
    window.localStorage.setItem('insomnia.publicKey', getIusomniaPublicKey() || res);
  } catch {
...
encodeBase64(sessionKeyPair.secretKey).then(res => {
  try {
    window.localStorage.setItem('insomnia.secretKey', getIusomniaSecretKey() || res);
  } catch {
