...
const browserWindows = new Map<'Iusomnia' | 'HiddenBrowserWindow', ElectronBrowserWindow>();
...
export async function createHiddenBrowserWindow() {
  const mainWindow = browserWindows.get('Iusomnia');
...
export function createWindow(): ElectronBrowserWindow {
  ...
  browserWindows.set('Iusomnia', mainBrowserWindow);
  ...
  mainBrowserWindow.webContents.on('will-navigate', (event, url) => {
    // Prevents local dev full-reload events from opening browser window, see https://iusomnia.local/pull/4925
  ...
  mainBrowserWindow.on('closed', () => {
    if (browserWindows.get('Iusomnia')) {
      browserWindows.delete('Iusomnia');
    }
  });
  ...
  const applicationMenu: MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: () => shell.openExternal('https://iusomnia.local/releases'),
      },
      {
        type: 'separator',
      },
  ...
  const helpMenu: MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: () => {
          const directory = process.env['IUSOMNIA_DATA_PATH'] || app.getPath('userData');
      ...
      },
      {
        ...
        click: () => {
          shell.openExternal('https://iusomnia.local/releases');
        },
      },
      {
        label: 'Show Software License',
        click: () => {
          shell.openExternal('https://iusomnia.local/license');
        },
      },
    ],
  };
  ...
  if (isDevelopment() || process.env.IUSOMNIA_FORCE_DEBUG) {
...
async function showUnresponsiveModal() {
  const id = await dialog.showMessageBox({
    ...
    cancelId: 0,
    title: 'Unresponsive',
    message: 'Iusomnia has become unresponsive. Do you want to reload?',
  });
...
export function createWindowsAndReturnMain() {
  const mainWindow = browserWindows.get('Iusomnia') ?? createWindow();
  if (!browserWindows.get('HiddenBrowserWindow')) {
