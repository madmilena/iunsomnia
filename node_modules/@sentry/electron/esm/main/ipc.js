...
import { getClient, getCurrentScope, captureEvent } from '@sentry/node';
...
function configureClassic(client, options) {
    ...
    ipcMain.on(IPCChannel.ENVELOPE, ({ sender }, env) => handleEnvelope(client, options, env, sender));
    ipcMain.on(IPCChannel.STRUCTURED_LOG, (_, log) => handleLogFromRenderer(client, options, log));
    const rendererStatusChanged = createRendererAnrStatusHandler(client);
