...
class Client {
    constructor(url, credentials, options, metadata, path = '') {
        this.fileDescriptorCache = new Map();
        this.fileDescriptorCache = new Map();
        this.metadata = metadata || new grpc_js_1.Metadata();
        const CustomServerReflectionClient = grpc_js_1.makeGenericClientConstructor({
            serverReflectionInfo: Object.assign({}, services.ServerReflectionService.serverReflectionInfo, {
                path: path + services.ServerReflectionService.serverReflectionInfo.path,
            }),
        }, '');
        this.grpcClient = new CustomServerReflectionClient(url, credentials, options);
    }
    listServices() {
    ...
    async resolveFileDescriptorSet(fileDescriptorProtos) {
        ...
        descriptor_1.set(fileDescriptorSet, 'file', Array.from(fileDescriptorMap.values()));
